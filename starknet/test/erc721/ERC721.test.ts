...
describe("ERC721 Test Cases", function () {
  ...
  /*
		ContractOwner: The 'owner' of the contract, as defined by the ownable.cairo spec
  ...
  it("ContractOwner can mint NFTs to a different address", async function () {
    await tryCatch(async () => {
      ...
      await acc1.invoke(contract, "mint", { to: toWallet, tokenId: num0 });
      ...
      await acc1.invoke(contract, "mint", { to: toWallet, tokenId: num1 });
      ...
      await acc1.invoke(contract, "mint", { to: toWallet, tokenId: num2 });
      ...
      await acc1.invoke(contract, "mint", { to: toWallet, tokenId: num3 });
      ...
      await acc1.invoke(contract, "mint", { to: toWallet, tokenId: num4 });
  ...
  it("NOT_NFTOwner can NOT burn someone elses NFT", async function () {
    await tryCatch(async () => {
      const tokenIdUint256 = toUint256WithFelts("0");

      // We use acc1 to invoke burn, which is not the owner of NFT with id 0, hence should fail
      await shouldFail(
        acc1.invoke(contract, "burn", { tokenId: tokenIdUint256 }),
        "ERC721: caller is not the token owner"
      );
    });
  });

  it("NFTOwner can burn their NFT", async function () {
    await tryCatch(async () => {
      const acc2Address = BigInt(acc2.starknetContract.address);
      // burn tokenId 0
      const tokenIdUint256 = toUint256WithFelts("0");

      // Get balance of account2 before burning and calculate expected balance
      const originalBalance = (
        await contract.call("balanceOf", { owner: acc2Address })
      ).balance;
      const expectedBalance = (
        parseInt(fromUint256WithFelts(originalBalance).toString()) - 1
      ).toString();

      // invoke burn as acc2 which owns NFT with id 0 and should pass
      await acc2.invoke(contract, "burn", { tokenId: tokenIdUint256 });
      const balance = (await contract.call("balanceOf", { owner: acc2Address }))
        .balance;

      // Balance of acc2 should be old balance - 1
      expect(balance).to.deep.equal(toUint256WithFelts(expectedBalance));
    });
  });
