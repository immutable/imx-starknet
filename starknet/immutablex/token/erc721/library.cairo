...
# - Remove tokenURI and related functions to handle separately in token/erc721/erc721_token_metadata
# - Rename _exists -> ERC721_exists to mark it as an importable function rather than internal only
...
from immutablex.utils.constants import (
    IERC721_ID, IERC721_METADATA_ID, IACCOUNT_ID, IERC721_RECEIVER_ID)
...
func ERC721_only_token_owner{pedersen_ptr : HashBuiltin*, syscall_ptr : felt*, range_check_ptr}(
        token_id : Uint256):
    uint256_check(token_id)
    let (caller) = get_caller_address()
    let (owner) = ERC721_ownerOf(token_id)
